...
for i in range(len(blocks)):
        ...
        offset = x2
        f.write("$ggen1 nbl=%d,x1min=%.6e,x1max=%.6e,igrid=%d,x1rat=%.3f,lgrid=.%s. /\n"%(n, x1, x2, dir, r, done))

dx1 = (starthighres + s) - startingpoint
x10 = starthighres + (3./2.)*s

f.write("$ggen2 nbl=%d,x2min=%.6e,x2max=%.6e,igrid=%d,x2rat=%.3f,lgrid=.false. /\n" %(x2_blk[0], x2_min1, s * -1, -1, x2_r[0]))
f.write("$ggen2 nbl=%d,x2min=%.6e,x2max=%.6e,igrid=%d,x2rat=%.3f,lgrid=.false. /\n" %(64, s * -1, s, 1, 1))
f.write("$ggen2 nbl=%d,x2min=%.6e,x2max=%.6e,igrid=%d,x2rat=%.3f,lgrid=.true. /\n" %(x2_blk[1], s, x2_max3, 1, x2_r[1]))
f.write("$ggen3 nbl=%d,x3min=%.6e,x3max=%.6e,igrid=%d,x3rat=%.3f,lgrid=.false. /\n" %(x3_blk[0], x3_min1, s * -1, -1, x3_r[0]))
f.write("$ggen3 nbl=%d,x3min=%.6e,x3max=%.6e,igrid=%d,x3rat=%.3f,lgrid=.false. /\n" %(64, s * -1, s, 1, 1))
f.write("$ggen3 nbl=%d,x3min=%.6e,x3max=%.6e,igrid=%d,x3rat=%.3f,lgrid=.true. /\n" %(x3_blk[1], s, x3_max3, 1, x3_r[1]))
f.write(' &grvcon /\n')
f.write(' &radcon /\n')
f.write(' &eqos gamma=1.4,mmw=1.0D0 /\n')
f.write(' &pgen r=%.6e, x10=%.6e , d0=0.917, e0=0.6, v11=%.2e, e1=0.0, \n'%(s/2., x10, v * -1))
